#!/usr/bin/env python3
"""
Builds /conjuring-arts/ from data/conjuring-arts.csv.
...
OUTDIR  = os.path.join(SITE, "conjuring-arts")
IMGDIR  = os.path.join(SITE, "assets", "img", "conjuring")
IMGWEB  = "/assets/img/conjuring"
CSSHREF = "/assets/css/conjuring.css"
BASEURL = "https://zibalion.com/conjuring-arts"

SECTION_TITLE = "Conjuring Arts"
LEDE = ("Books on conjuring, sleight of hand and deception, most of them long "
        "out of print. What follows is what is actually on the shelf.")
...
def build_index(rows, head, tail):
    ...
    for sec, books in sections:
        ...
        for b in books:
            line = f'      <li><a href="/conjuring-arts/{b["slug"]}/">{esc(b["title"])}</a>'
            if (b.get("author") or "").strip():
                line += f' <span class="cj-by">{esc(b["author"])}</span>'
            if (b.get("volumes") or "").strip():
                line += f' <span class="cj-vol">{esc(b["volumes"])}</span>'
            out.append(line + "</li>\n")
        out.append("    </ul>\n  </section>\n")
...
def build_book(b, head, tail):
    ...
    out.append(f'  <a class="cj-back" href="/conjuring-arts/">Back to {SECTION_TITLE}</a>\n')
    out.append("</main>\n\n")
    out.append(tail)
...
def main():
    ...
    print(f"Built /conjuring-arts/ and {len(rows)} book pages.")
